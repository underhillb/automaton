class ConfigurationTemplatesController < ApplicationController
  ...
  def index
    ...
    def versions
      @div,template=params[:blueprint][:blueprint_objects_attributes].first
      template_id=template.values[0]
      @versions=ConfigurationTemplate.find(template_id).configuration_files
